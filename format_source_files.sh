...
# 	find . -type f -name '*.cpp' -o -name '*.h'
	git ls-tree -r $(git status  | head -1 | awk '{print $3}')  --name-only | grep "\.cpp\|\.h"
fi | \
...
while read filename ; do
	...
	if [[ ! -z $( echo ${filename} | grep "\/external\/" ) ]]; then
    echo "${MY_NAME}: Skip external source: \"${filename}\"" >&2
		continue
	fi

	TEMPFILE=$(mktemp -t 'calyp_formatted_source_XXXXXXXXXXX.cpp')
...
# 	astyle -n --align-reference=name --align-pointer=name --indent=spaces=4 --indent-labels --pad-oper --unpad-paren --pad-header --keep-one-line-statements --convert-tabs --indent-preprocessor <"${filename}" >${TEMPFILE}
  $CLANG_FORMAT -style=file ${filename} > ${TEMPFILE}
